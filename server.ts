...
void app.prepare().then(() => {
  ...
  io.on("connection", (socket) => {
      ...
      meshState = meshState.messages.some(message => message.id === event.reportId)
        ? {
            ...
            messages: meshState.messages.map(message => {
              ...
              if (previous === event.direction) return { ...message, upvotes: message.upvotes - (event.direction === "up" ? 1 : 0), downvotes: message.downvotes - (event.direction === "down" ? 1 : 0) };
              return { ...message, upvotes: message.upvotes + (event.direction === "up" ? 1 : previous === "up" ? -1 : 0), downvotes: message.downvotes + (event.direction === "down" ? 1 : previous === "down" ? -1 : 0) };
            }),
          }
        : meshState;
    } else {
    ...
    if (event.kind === "report" && event.groups.length) socket.emit("pazabit:feedback", { title: "Report delivered", detail: `Your report was delivered to ${event.groups.join(" and ")}.` });
    if (event.kind === "escalation") socket.emit("pazabit:feedback", { title: "Report delivered", detail: `The report was delivered to ${event.groups.join(" and ")}.` });
    if (event.kind === "group-created") socket.emit("pazabit:feedback", { title: "Private group created", detail: `${event.group.tag} is ready for encrypted coordination.` });
    });
