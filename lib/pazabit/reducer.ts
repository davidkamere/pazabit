...
export function applyPazabitEvent(state: PazabitMeshState, event: PazabitEvent): PazabitMeshState {
  switch (event.kind) {
    case "report": return { ...state, messages: [{ id: event.eventId, time: "now", body: event.body, flag: event.flag, upvotes: 0, downvotes: 0, groups: event.groups, arrival: event.arrival }, ...state.messages] };
