...
export function CreateGroupModal({
  ...
  onClose: () => void;
  onCreate: (tag: string, password: string) => void;
}) {
  const [tag, setTag] = useState("");
  const [password, setPassword] = useState("");
  const normalized = tag ? `#${tag.replace(/^#/, "").replace(/\s/g, "")}` : "";
  return (
    <div className="fixed inset-0 z-30 grid place-items-center bg-black/60 p-4 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-2xl border border-[#3b4140] bg-[#171a1a] p-6 shadow-2xl">
        <div className="flex justify-between">
          <div>
            <p className="text-xs font-bold uppercase tracking-[.14em] text-lime">Private by default</p>
            <h3 className="mt-2 text-xl font-bold tracking-[-.04em]">Create action group</h3>
          </div>
          <button onClick={onClose} className="text-muted hover:text-white">
            <Icon name="close" className="size-5" />
          </button>
        </div>
        <p className="mt-3 text-sm leading-6 text-muted">Groups are encrypted and require a password to join.</p>
        ...
        <label className="mt-4 block text-xs font-bold uppercase tracking-wide text-muted">
          ...
          <input
            ...
            className="mt-2 w-full rounded-xl border border-line bg-[#1e2222] px-3 py-3 text-sm text-white outline-none placeholder:text-[#626967] focus:border-lime"
          />
        </label>
        <button
          onClick={() => normalized && onCreate(normalized, password)}
          disabled={!normalized || !password}
